...
const getCustomFields = (product) => {
    let fields = [
        ...
        { "name": "shop_runner_eligible", "value": validateValue(product?.["sr_eligible"]?.["en"], "boolean") },   
        { "name": "search_placement", "value": validateValue(product?.["search-placement"], "string") },
        { "name": "is_coming_soon", "value": validateValue(product?.["isComingSoon"]?.["en"], "boolean") },   
        { "name": "is_new", "value": validateValue(product?.["isNew"]?.["en"], "boolean") },   
        { "name": "model_number", "value": validateValue(product?.["modelNumber"]?.["en"], "string") },   
        { "name": "new_colours", "value": validateValue(product?.["newColours"]?.["en"], "boolean") },   
    ...
    try {
        ...
        for(let variant of productVariants){
            ...
            let obj = {
                ...
                price: variant.price,
                upc: variant["product-id"]?.toString()
            };
            if(optionValues){
                obj.option_values = optionValues;
                variantArr.push(obj);
            }
        }
        return variantArr;
...
const getOptionValues = (variantData) => {
    ...
    for(let key of Object.keys(variantData)){
        ...
        switch(keyLower){
            case "color": 
                let colorName = getColorName(variantData[key]["en"]);
                if(colorName){
...
const getCategoriesIds = async (product) => {
    ...
    // add secondary use case to sports as we have sports options only there
    let sports = [ sport, ...secondaryEndUse].filter(v => v);
    ...
    let menCatg = categories.find(cat => cat.name.toLowerCase().includes("shop men"));
    let menCatgId = menCatg.id;
    let womenCatg = categories.find(cat => cat.name.toLowerCase().includes("shop women"));
    ...
    if(productTypeMapped){
        // use only one level tree to get gender specific productTypes
    ...
    // get categories based on product sport and gender
    let genderSportCategoriesIds = [];
    let productTypeCatInsideGenderSportsIds = [];
    if(sports.length){
        let genderSportCategories = genderFilteredCategories.filter(cat => sports.some(sport => cat.name.toLowerCase().includes(sport)));
        genderSportCategoriesIds = genderSportCategories.map(obj => obj.id);
        console.log("genderSportCategoriesIds: ", genderSportCategoriesIds)
        
        // get all categories which are given gender sports or have gender sports as a parent category 
        let genderSportFilteredCategories = genderFilteredCategories.filter(cat => [genderSportCategoriesIds].includes(cat.id) || genderSportCategoriesIds.some(id => cat.parentIds.includes(id)))
        console.log("genderSportFilteredCategories -> ", genderSportFilteredCategories)
        // get product categories which are sub category of gender activity 
        let productTypeCatInsideGenderSports = genderSportFilteredCategories.filter(cat => [productTypeMapped, productType].includes(cat.name.toLowerCase()))
        console.log("productTypeCatInsideGenderSports", productTypeCatInsideGenderSports);
        productTypeCatInsideGenderSportsIds = productTypeCatInsideGenderSports.map(obj => obj.id);
    }

    // get categories based on product sport
    let shopBySportsCategory = 
        categories.find(cat => cat.name.toLowerCase().includes("shop by sports"));
    let shopBySportCatId = shopBySportsCategory.id;
    let shopBySportsFilteredCategories = 
        categories.filter( cat => cat.id == shopBySportCatId || cat.parentIds.includes(shopBySportCatId) );
    let sportCategoriesIds = [];
    let productTypeCatInsideSportsIds = [];
    if(sports.length){
        categoryIds = [ ...categoryIds, shopBySportCatId ];
        let sportCategories = 
            shopBySportsFilteredCategories.filter(cat => sports.some(sport => cat.name.toLowerCase().includes(sport)));
    ...
    // get categories based on 'secondary end use' and gender field
    ...
    // get categories based on 'secondary end use' and sports
    // console.log("secondaryEndUse:", secondaryEndUse);
    // let secondaryEndUseCategoriesIds = [];
    // if(secondaryEndUse){
    //     let secondaryEndUseCategories = categories.filter(cat => secondaryEndUse.some(el => cat.name.toLowerCase().includes(el)))
    ...
    categoryIds = [ 
        ...categoryIds, 
        ...productTypeCatInsideGenderIds, 
        ...genderSportCategoriesIds,
        ...productTypeCatInsideGenderSportsIds,
        // ...secondaryEndUseCategoriesIds, 
        ...sportCategoriesIds, 
        ...productTypeCatInsideSportsIds,
        ...fabricTechCategoriesIds];
